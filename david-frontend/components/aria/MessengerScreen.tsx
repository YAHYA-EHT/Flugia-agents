...
import type { MessengerStatus } from "@/lib/aria/types";
import { SkeletonCard } from "./Skeleton";

export function MessengerScreen({ onGoToEmail }: { onGoToEmail?: () => void }) {
  const [info, setInfo] = useState<MessengerStatus | null>(null);
  const [loading, setLoading] = useState(true);
  const pollRef = useRef<ReturnType<typeof setInterval> | null>(null);

  const load = async () => {
    try { setInfo(await getApi().getMessengerStatus()); }
  ...
  // While waiting for the user to tap the m.me link, poll for the link to complete.
  useEffect(() => {
    ...
    <div className="h-full overflow-y-auto bg-slate-50 dark:bg-slate-950">
      ...
      <div className="sticky top-0 z-10 border-b border-slate-100 bg-white px-6 py-3.5 dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center gap-3">
          <Image src="/messenger-logo.webp" alt="Messenger" width={36} height={36} className="h-9 w-9 shrink-0 object-contain" />
          <div>
            <h1 className="text-sm font-bold leading-tight text-slate-900 dark:text-slate-100">Messenger</h1>
            <p className="mt-0.5 text-[11px] text-slate-400">Message your assistant from Messenger</p>
          </div>
        </div>
      </div>
...
// Hard gate: no Messenger without a connected email.
  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center rounded-2xl border-2 border-dashed border-slate-200 bg-white p-10 text-center dark:border-slate-800 dark:bg-slate-900">
      ...
      <p className="mt-2 text-sm leading-relaxed text-slate-500 dark:text-slate-400">
        Messenger relays your inbox and calendar, so your assistant needs a connected email
...
function Manager({ info, reload }: { info: MessengerStatus; reload: () => Promise<void> }) {
  ...
  async function connect() {
    setError(null);
    setConnecting(true);
    try { await getApi().createMessengerLink(); await reload(); }
  ...
  async function unlink() {
    setUnlinking(true);
    try { await getApi().unlinkMessenger(); await reload(); }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <p className="text-sm leading-relaxed text-slate-500 dark:text-slate-400">
        Connect Messenger to message your assistant from there. One tap opens a chat with the
        page — no phone number or code needed.
      </p>
      ...
      {info.status === "not_linked" && (
        <div>
          <div className="flex flex-col items-center rounded-2xl border-2 border-dashed border-slate-200 py-12 text-center dark:border-slate-800">
            <Image src="/messenger-logo.webp" alt="" width={40} height={40} className="h-10 w-10 object-contain opacity-40 grayscale" />
            <p className="mt-3 text-sm font-medium text-slate-400">Not connected yet</p>
            <button
              onClick={() => void connect()}
              disabled={connecting}
              className="mt-4 flex items-center gap-2 rounded-xl bg-[#0084FF] px-5 py-2.5 text-sm font-medium text-white transition hover:opacity-90 disabled:opacity-50"
            >
      ...
      {info.status === "pending_link" && info.link && (
        <div className="rounded-2xl border border-slate-100 bg-white p-5 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#0084FF]/10 text-[#0084FF]">
              <MessageCircle className="h-4 w-4" />
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Almost there</p>
              <p className="text-xs text-slate-400">Open the link, then tap Get Started / send a message</p>
            </div>
          ...
          <div className="mt-4 flex gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="flex flex-1 items-center justify-center gap-2 rounded-xl bg-[#0084FF] px-4 py-2.5 text-sm font-medium text-white transition hover:opacity-90"
            >
      ...
      {info.status === "active" && (
        <div className="rounded-2xl border border-slate-100 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-3">
            <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#0084FF]/10 text-[#0084FF]">
              <MessageCircle className="h-4 w-4" />
            </span>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">Messenger connected</p>
